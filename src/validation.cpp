                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s WDC additional fees, %d delta bytes\n",
...
static bool ReadBlockOrHeader(T& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // A different rule for the genesis block. Because the original WDC genesis wan not mined correctly.
...
CAmount GetWDCSubsidy(int nHeight, const Consensus::Params& consensusParams) 
{
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight < consensusParams.nDiffChangeTarget){
        ...
        nSubsidy = GetWDCSubsidy(nHeight, consensusParams);
    }
	//make sure the reward is at least 1 WDC
    if (nSubsidy < COIN) nSubsidy = COIN;

    return nSubsidy;
    // block rewards are now defined in src/worldcoin.cpp
    //return GetWorldcoinBlockSubsidy(nHeight, consensusParams);
}
        ...
        if (fScriptChecks) {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (pvChecks) {
                    ...
                    // WDC-DEV note: that softfork is not the case of WDC network - SANDO re-enabled
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // WDC dev note: some valid blocks fails this test so temporary bypass it untill resolve the reason
    // only log a warnings
    if (!control.Wait())
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Worldcoin: Disallow AuxPow blocks before it is activated.
    ...
    // Worldcoin: Version 2 enforcement was never used
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    // Worldcoin: We probably want to disable this
    int nLockTimeFlags = 0;
